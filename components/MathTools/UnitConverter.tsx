...
export default function UnitConverter({ activeTool }: { activeTool: string }) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 animate-in fade-in duration-300">
      ...
      <p className="text-neutral-500 dark:text-neutral-400 text-sm mb-6">Convert length, weight, and area with high precision.</p>

      <div className="mb-6">
        <label className="block text-xs font-bold mb-2 dark:text-neutral-300">Measurement Category</label>
        <select value={category} onChange={(e) => handleCategoryChange(e.target.value)} className="w-full md:w-64 p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-lg dark:text-white font-medium">
      ...
      <div className="flex flex-col md:flex-row items-end gap-3 mb-6">
        <div className="flex-1 w-full">
          ...
          <input type="number" value={valFrom} onChange={(e) => setValFrom(e.target.value === "" ? "" : Number(e.target.value))} className="w-full p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-lg dark:text-white font-medium" />
        </div>
        <div className="flex-1 w-full">
          <label className="block text-xs font-bold mb-2 dark:text-neutral-300">From</label>
          <select value={unitFrom} onChange={(e) => setUnitFrom(e.target.value)} className="w-full p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-lg dark:text-white font-medium">
            {units.map(u => <option key={`from-${u}`} value={u}>{u}</option>)}
          </select>
        </div>
        <div className="flex-1 w-full">
          <label className="block text-xs font-bold mb-2 dark:text-neutral-300">To</label>
          <select value={unitTo} onChange={(e) => setUnitTo(e.target.value)} className="w-full p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-lg dark:text-white font-medium">
            {units.map(u => <option key={`to-${u}`} value={u}>{u}</option>)}
          </select>
        </div>
      </div>

      <div className="bg-neutral-100 dark:bg-neutral-800 p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
        <span className="text-sm font-bold text-neutral-600 dark:text-neutral-300">Converted Value:</span>
        <span className="text-2xl md:text-3xl font-mono font-bold text-orange-600 dark:text-sky-400 break-all text-right">{convert()} <span className="text-lg text-neutral-500">{unitTo}</span></span>
      </div>
    </div>
  );
}