...
export default function JpgTools({ activeTool, isDark }: { activeTool: string, isDark: boolean }) {
  if (activeTool !== "jpg-tools") return null;

  const [imgSrc, setImgSrc] = useState<string | null>(null);
  const [imgName, setImgName] = useState("image");
  const [quality, setQuality] = useState(85);
  const [bgColor, setBgColor] = useState("#ffffff"); // Matte background for transparency
  ...
  const processJpg = (source: string, qual: number, background: string) => {
    ...
    img.onload = () => {
      ...
      // Fill background color (JPEGs don't support alpha transparency)
      ...
      // Calculate approximate file size from Base64 string length
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="text-[11px] text-slate-400">Replaces transparent pixels</span>
              </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex-1 min-h-[250px] bg-slate-100 dark:bg-slate-900 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl flex items-center justify-center overflow-hidden relative" style={checkerboardStyle}>
            {previewUrl ? (
              ...
              <span className="text-sm font-semibold text-slate-400">Upload an image to optimize</span>
            )}
          </div>
        </div>
      </div>
