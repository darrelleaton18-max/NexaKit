    ...
    <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 animate-in fade-in duration-300">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 dark:text-neutral-400 text-sm leading-relaxed mb-4 max-w-3xl">
        ...
        {/* Currency Toggle */}
        <div className="flex bg-neutral-100 dark:bg-neutral-800 p-1 rounded-lg border border-neutral-200 dark:border-neutral-700 shrink-0 self-start">
          {["GBP", "USD", "EUR"].map(c => (
            <button 
              key={c}
              onClick={() => {
              ...
              className={`px-4 py-2 rounded-md text-sm font-bold transition-colors flex items-center gap-2 ${currency === c ? "bg-white dark:bg-neutral-700 shadow-sm text-orange-600 dark:text-sky-400" : "text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300"}`}
            >
              {c === "GBP" ? "£" : c === "USD" ? "$" : "€"} {c}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-xl border border-neutral-200 dark:border-neutral-700">
            <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-200 uppercase tracking-widest mb-4">Data Ingestion</h3>
            
            <input type="file" accept=".csv" onChange={handleFileUpload} ref={fileInputRef} className="hidden" />
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="w-full py-4 mb-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-xl transition-colors shadow-sm flex items-center justify-center gap-2"
            >
            ...
            <div className="relative flex items-center py-2 mb-4">
              ...
              <span className="flex-shrink-0 mx-4 text-xs font-bold text-neutral-400 uppercase">OR</span>
              <div className="flex-grow border-t border-neutral-200 dark:border-neutral-700"></div>
            </div>

            <button onClick={loadSampleData} className="w-full py-3 bg-neutral-200 hover:bg-neutral-300 dark:bg-neutral-700 dark:hover:bg-neutral-600 text-neutral-700 dark:text-white font-bold rounded-xl transition-colors text-sm">
          ...
          {/* BENCHMARKING PANEL */}
          <div className="bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-xl border border-neutral-200 dark:border-neutral-700">
            <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-200 uppercase tracking-widest mb-4">Market Benchmarking</h3>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-4">Contrasts current financial institution costs against regional standards.</p>
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-neutral-600 dark:text-neutral-300">Standard Account Fee</span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400">{formatMoney(0)} - {formatMoney(5)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-neutral-600 dark:text-neutral-300">Intl. Transaction Rate</span>
        ...
        <div className="lg:col-span-2">
          {isProcessing ? (
            <div className="h-full min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl">
              <div className="w-10 h-10 border-4 border-orange-200 border-t-orange-600 rounded-full animate-spin mb-4"></div>
              <p className="text-neutral-500 dark:text-neutral-400 font-bold animate-pulse">Running Multi-Lingual Auditor Engine...</p>
            </div>
          ) : totalScanned === 0 ? (
            <div className="h-full min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl bg-neutral-50/50 dark:bg-neutral-800/20 text-center p-8">
              <svg className="w-16 h-16 text-neutral-300 dark:text-neutral-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
              <h3 className="text-lg font-bold text-neutral-700 dark:text-neutral-300 mb-2">Awaiting Financial Statement</h3>
              <p className="text-sm text-neutral-500 dark:text-neutral-400 max-w-sm">Upload a CSV statement or load the sample data to initiate the detection algorithms.</p>
            </div>
          ) : (
            <div className="animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-xl border border-orange-100 dark:border-orange-800/50">
                  <span className="block text-xs font-bold text-orange-600 dark:text-orange-400 uppercase mb-1">Total Recoverable</span>
                  <span className="text-2xl font-black text-neutral-800 dark:text-white">{formatMoney(totalRecoverable)}</span>
                </div>
                <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-100 dark:border-red-800/50">
                  ...
                  <span className="text-2xl font-black text-neutral-800 dark:text-white">{feeCount} Detected</span>
                </div>
                <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-100 dark:border-amber-800/50">
                  ...
                  <span className="text-2xl font-black text-neutral-800 dark:text-white">{subCount} Detected</span>
                </div>
              </div>

              {/* FLAGGED ITEMS TABLE */}
              <div className="overflow-hidden border border-neutral-200 dark:border-neutral-700 rounded-xl">
                <div className="bg-neutral-100 dark:bg-neutral-800 px-4 py-3 border-b border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
                  <h3 className="font-bold text-sm text-neutral-700 dark:text-neutral-200">Flagged Anomalies</h3>
                  <span className="text-xs font-bold bg-white dark:bg-neutral-700 px-2 py-1 rounded text-neutral-500 dark:text-neutral-300 border border-neutral-200 dark:border-neutral-600">
                    {flags.length} findings out of {totalScanned} rows
                  </span>
                </div>
                
                {flags.length === 0 ? (
                  <div className="p-8 text-center text-emerald-600 dark:text-emerald-400 font-bold bg-neutral-50 dark:bg-neutral-900">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      ...
                      <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 bg-white dark:bg-neutral-900">
                        {flags.map(f => (
                          <tr key={f.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors group">
                            <td className="p-4 font-mono text-xs text-neutral-500 dark:text-neutral-400 whitespace-nowrap">{f.date}</td>
                            <td className="p-4">
                              <span className="font-bold text-neutral-800 dark:text-neutral-200 block">{f.description}</span>
                              <span className="text-xs text-neutral-500 dark:text-neutral-400 hidden group-hover:block mt-1">{f.notes}</span>
                            </td>
                            <td className="p-4">
                            ...
                            <td className="p-4 text-right font-bold text-neutral-800 dark:text-white whitespace-nowrap">
