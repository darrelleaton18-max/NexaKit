...
export default function SavingsGoal({ activeTool }: { activeTool: string }) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 animate-in fade-in duration-300">
      ...
      <h2 className="text-2xl font-bold mb-1 dark:text-white">Rainy-Day Emergency Fund</h2>
      <p className="text-neutral-500 dark:text-neutral-400 text-sm mb-3">Set a target, track your deposits, and celebrate your milestones.</p>
      ...
      <div className="bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 mb-8">
        <div className="flex justify-between items-end mb-4">
          <div>
            <span className="block text-sm font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider mb-1">Current Savings</span>
            <span className="text-4xl font-black text-emerald-600 dark:text-emerald-400">{formatMoney(current)}</span>
          </div>
          <div className="text-right">
            <span className="block text-xs font-bold text-neutral-400 uppercase mb-1">Target Goal</span>
            <span className="text-xl font-bold dark:text-neutral-300">{formatMoney(target)}</span>
          </div>
        </div>

        <div className="w-full bg-neutral-200 dark:bg-neutral-900 rounded-full h-6 mb-2 overflow-hidden shadow-inner border border-neutral-300 dark:border-neutral-950">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <div>
          <label className="block text-xs font-bold mb-2 dark:text-neutral-300">Update Goal Target (£)</label>
          <input type="number" value={target} onChange={e => setTarget(Number(e.target.value))} className="w-full p-4 text-xl font-bold border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-xl dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none" />
        </div>
        <div>
          <label className="block text-xs font-bold mb-2 dark:text-neutral-300">Add to Savings</label>
          <div className="flex flex-col sm:flex-row gap-2">
            <input type="date" value={addDate} onChange={e => setAddDate(e.target.value)} className="p-4 font-bold border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-xl dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none" />
            <input type="number" value={addAmount} onChange={e => setAddAmount(e.target.value === "" ? "" : Number(e.target.value))} placeholder="Amount (£)" className="flex-1 w-full p-4 text-xl font-bold border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 rounded-xl dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none" />
      ...
      {history.length > 0 && (
        <div className="animate-in fade-in duration-300">
          <div className="flex justify-between items-center mb-4 px-2">
            ...
            <span className="text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">{history.length} Transactions</span>
          </div>
          ...
          <div className="overflow-hidden border border-neutral-200 dark:border-neutral-700 rounded-xl">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 font-mono bg-neutral-50 dark:bg-neutral-900">
                {history.map(h => (
                  <tr key={h.id} className="dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => handleDelete(h.id)} className="text-neutral-400 hover:text-red-500 font-bold transition-colors">×</button>
                    </td>
                  </tr>
                ))}
              </tbody>
