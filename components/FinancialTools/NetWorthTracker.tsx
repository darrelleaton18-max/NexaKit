...
export default function NetWorthTracker({ activeTool }: { activeTool: string }) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 animate-in fade-in duration-300">
      ...
      <p className="text-neutral-500 dark:text-neutral-400 text-sm mb-3">Track your total assets and liabilities.</p>
      ...
      <div className={`p-6 rounded-xl border mb-8 text-center ${netWorth >= 0 ? 'bg-emerald-50 border-emerald-200 dark:bg-emerald-900/30' : 'bg-red-50 border-red-200 dark:bg-red-900/30'}`}>
        <span className="block text-sm font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400 mb-1">Total Net Worth</span>
      ...
      <div className="flex flex-col md:flex-row gap-2 mb-8 bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-xl border border-neutral-200 dark:border-neutral-700">
        <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Savings Account" className="flex-1 p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-900 rounded-lg dark:text-white" />
        <input type="number" value={newValue} onChange={e => setNewValue(e.target.value === "" ? "" : Number(e.target.value))} placeholder="Amount (£)" className="w-full md:w-32 p-3 border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-900 rounded-lg dark:text-white" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          ...
          <div className="space-y-2">
            {assets.map(a => (
              <div key={a.id} className="flex justify-between items-center p-3 bg-neutral-50 dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700">
                <span className="dark:text-neutral-300 font-medium">{a.name}</span>
        ...
        <div>
          ...
          <div className="space-y-2">
            {liabilities.map(l => (
              <div key={l.id} className="flex justify-between items-center p-3 bg-neutral-50 dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700">
                <span className="dark:text-neutral-300 font-medium">{l.name}</span>
                <div className="flex items-center gap-4">
