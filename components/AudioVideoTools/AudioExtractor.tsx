...
export default function AudioExtractor() {
  ...
  // Load the single-threaded FFmpeg core on mount to bypass GitHub Pages strict headers
  useEffect(() => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setVideoFile(file);
      setAudioUrl(null); // Reset previous outputs
      setProgress(0);
    }
  };
  ...
  const extractAudio = async () => {
    ...
    try {
      // 1. Write the video file to FFmpeg's virtual memory
      await ffmpeg.writeFile("input.mp4", await fetchFile(videoFile));

      // 2. Run the extraction command: -vn removes video, -acodec libmp3lame encodes to MP3
      await ffmpeg.exec(["-i", "input.mp4", "-vn", "-acodec", "libmp3lame", "output.mp3"]);

      // 3. Read the result back from virtual memory
      const fileData = await ffmpeg.readFile("output.mp3");
      
      // 4. Create a downloadable URL (cast to any to bypass strict TS rules)
      const url = URL.createObjectURL(new Blob([fileData as any], { type: "audio/mp3" }));
      setAudioUrl(url);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-300">
      <div>
        <h2 className="text-3xl font-extrabold text-neutral-900 dark:text-white mb-2 tracking-tight">Audio Extractor</h2>
        <p className="text-neutral-500 dark:text-neutral-400">Extract MP3 audio from any video file instantly. Processed 100% locally in your browser.</p>
      </div>
      ...
      {videoFile && !audioUrl && (
        <div className="flex flex-col gap-4">
          ...
          >
            {isProcessing ? "Extracting Audio..." : "Extract MP3 Now"}
          </button>
      ...
      {audioUrl && (
        <div className="bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-3xl p-6 md:p-8 flex flex-col items-center animate-in zoom-in duration-300">
          ...
          <h3 className="text-xl font-bold text-neutral-900 dark:text-white mb-6">Extraction Complete!</h3>
          
          <audio controls src={audioUrl} className="w-full max-w-md mb-6" />
          
          <a 
            href={audioUrl} 
            download={`extracted_audio_${videoFile?.name.replace(/\.[^/.]+$/, "")}.mp3`}
