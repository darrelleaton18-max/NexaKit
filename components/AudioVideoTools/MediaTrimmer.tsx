...
export default function MediaTrimmer() {
  ...
  const trimMedia = async () => {
    ...
    try {
      // Extract the original file extension (e.g., .mp4, .mp3, .webm)
      const ext = mediaFile.name.split('.').pop() || 'mp4';
      const inputName = `input.${ext}`;
      const outputName = `output.${ext}`;

      // Write the file to virtual memory
      await ffmpeg.writeFile(inputName, await fetchFile(mediaFile));

      // Run the trim command
      // -ss is start time, -to is end time, -c copy processes it instantly without quality loss
      await ffmpeg.exec([
        "-i", inputName,
      ...
      // Read the result back and cast to any to bypass strict TS Blob rules
      ...
      alert("An error occurred during trimming. Make sure the timestamps are within the video's duration.");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-300">
      <div>
        <h2 className="text-3xl font-extrabold text-neutral-900 dark:text-white mb-2 tracking-tight">Media Trimmer</h2>
        <p className="text-neutral-500 dark:text-neutral-400">Cut and crop audio or video files to exact timestamps instantly. Zero quality loss.</p>
      </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-[#1a1a1a] border border-neutral-200 dark:border-white/[0.05] rounded-3xl p-6">
              <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-2">Start Time (Seconds)</label>
              <input 
                type="number" 
                min="0"
                value={startTime}
            ...
            <div className="bg-white dark:bg-[#1a1a1a] border border-neutral-200 dark:border-white/[0.05] rounded-3xl p-6">
              <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-2">End Time (Seconds)</label>
              <input 
                type="number" 
                min={startTime + 1}
                value={endTime}
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              {isProcessing ? "Trimming File..." : "Trim Media Now"}
            </button>
      ...
      {outputUrl && (
        <div className="bg-green-50 dark:bg-[#1a1a1a] border border-green-200 dark:border-white/[0.05] rounded-3xl p-6 md:p-8 flex flex-col items-center animate-in zoom-in duration-300">
          ...
          <h3 className="text-xl font-bold text-neutral-900 dark:text-white mb-6">Trimming Complete!</h3>
          ...
          <a 
            href={outputUrl} 
            download={`trimmed_${mediaFile?.name}`}
